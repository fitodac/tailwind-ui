...
export const codeHighlight = (elmnt, mode) => {
  ...
  function tagMode(txt) {
    ...
    result = done + rest;
    result = "<code class=" + tagcolor + ">&lt;</code>" + result.substring(4);
    if (result.substr(result.length - 4, 4) == "&gt;") {
      result = result.substring(0, result.length - 4) + "<code class=" + tagcolor + ">&gt;</code>";
    }
    return `<code class="${tagnamecolor}">${result}</code>`;
  }
  ...
  function cssMode(txt) {
    ...
    rest = done + rest;
    rest = rest.replace(/{/g, "<code style=color:" + cssdelimitercolor + ">{</code>");
    rest = rest.replace(/}/g, "<code style=color:" + cssdelimitercolor + ">}</code>");
    for (i = 0; i < comment.arr.length; i++) {
  ...
  function cssPropertyValueMode(txt) {
    var rest = txt, done = "", s;
    rest = "<code style=color:" + cssdelimitercolor + ">:</code>" + rest.substring(1);
